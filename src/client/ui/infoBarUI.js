...
export function drawInfoBarUI(scene, gameWidth, gameHeight, shopWidth, infoBarHeight) {
  ...
  const heartX = gameWidth - shopWidth - 180;
  const heartY = 40;
  scene.heartImage = scene.add.image(heartX, heartY, 'heart').setDisplaySize(40, 40).setOrigin(0.5);
  ...
  scene.lifeBarBg.fillRect(heartX + 30, heartY - 15, 120, 30);
  scene.lifeBar = scene.add.graphics();
  scene.lifeBar.setDepth(1002);
  scene.lifeText = scene.add.text(heartX + 90, heartY, `${scene.playerLives} / ${scene.maxPlayerLives}`, {
    font: "20px Arial",
    fill: "#fff"
  }).setOrigin(0.5);
  ...
  const nuggetY = heartY + 40;
  const nuggetX = heartX;
  const goldTextX = nuggetX + 32;
  scene.nuggetImage = scene.add.image(nuggetX, nuggetY, 'nugget').setDisplaySize(32, 32).setOrigin(0.5);
...
export function updateLifeBar(scene) {
  ...
  scene.lifeBar.clear();
  const barWidth = 120 * (scene.playerLives / scene.maxPlayerLives);
  scene.lifeBar.fillStyle(0xff4444, 1);
  scene.lifeBar.fillRect(scene.heartImage.x + 30, scene.heartImage.y - 15, barWidth, 30);
